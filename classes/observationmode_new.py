...
class VergassolaPlumeModel(ObservationModel):
    ...
    def __init__(self, cfg):
        ...
        self.Nagents = int(cfg.get("domain/Nagents"))
        ...
        # Get all possible hits for the agent(s)
        self.hits = list(product(np.arange(self.Nhits), repeat=self.Nagents))
    ...
    def _get_hit_probabilities_meshgrid(self):
        ...
        NOTE: Hit probability is independent from the number of agents.
        ...
        # print(f"indices in get_probabilities_meshgrid in observationmodel = \n {indices}")
    ...
    def _get_hit_probabilities(self, agent, source, flux):
        ...
        # cconvert to numpy array for computation of d_over_dx and d_over_dx_downwind
    ...
    def _get_hit_probabilities_from_meshgrid(self, agent, source, flux_index):
        ...
        NOTE: This is for one agent, hit_probabilities is independent from the number of agents.
    ...
    def _get_observation(self, agent, source, flux, flux_index, seed):
        ...
        Returns
        -------
        observation: tuple of len Nagents
        """

        observation = [0] * self.Nagents

        for i, one_agent in enumerate(agent):

            # get hit probabilities
            hit_probabilities = self._get_hit_probabilities_from_meshgrid(one_agent, source, flux_index)
            # print(f"hit_probabilities = {hit_probabilities}")

            # this should be similar to get_hit_probabilities, but is more expensive to compute
            # assert np.all(hit_probabilities == self._get_hit_probabilities(one_agent, source, flux))

            # get an observation based on the hit probabilities
            rng = np.random.default_rng(seed)
            observation[i] = rng.choice(a=range(self.Nhits), p=hit_probabilities)
